...
class TokenTransactionsPrices:
  ...
  def _iterate_top_tokens_txs(self, addresses):
    query = {
      'query': {
        'bool': {
          'must': [
  ...
  def _get_prices_by_dates(self, dates, symbols):
    ids = []
    ...
    for price in res:
      if price['found'] == False:
        continue
      if ('USD' in price['_source'].keys()) and (price["_source"]["USD"] is not None):
        prices[price['_source']['token'] + '_' + price['_source']['timestamp']] = price['_source']['USD']
      elif ('USD_cmc' in price['_source'].keys())  and (price["_source"]["USD_cmc"] is not None):
        prices[price['_source']['token'] + '_' + price['_source']['timestamp']] = price['_source']['USD_cmc']
    return prices
  ...
  def extract_transactions_prices(self):
    ...
    for token_txs in self._iterate_top_tokens_txs(addresses):
      ...
      prices = self._get_prices_by_dates(dates, symbols)
      update_docs = []
      for tx in token_txs:
        ...
        try:
          ...
          usd_value = self._to_usd(value, prices[prices_key])
          usd_value = float('{:0.10f}'.format(usd_value))
        except Exception as e:
          print(tx['_source'])
          print('exception:', e)
          usd_value = None
          value = None
        update_doc = {'doc': {'usd_value': usd_value, 'timestamp': timestamp}, 'id': tx['_id']}
