...
class InputParsingTestCase(unittest.TestCase):
  def setUp(self):
    self.contracts = Contracts({"contract": TEST_CONTRACTS_INDEX, "transaction": TEST_TRANSACTIONS_INDEX})
  ...
  def test_iterate_transactions_by_targets(self):
    for i in tqdm(range(20)):
      self.client.index(TEST_TRANSACTIONS_INDEX, 'tx', {'to': TEST_CONTRACT_ADDRESS}, id=i + 1, refresh=True)
    for i in tqdm(range(20)):
      self.client.index(TEST_TRANSACTIONS_INDEX, 'tx', {'to': "0x"}, id=i + 21, refresh=True)
