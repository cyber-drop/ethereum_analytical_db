...
class ContractMethodsTestCase(unittest.TestCase):
  def setUp(self):
    ...
    try:
      ...
      pass
    self.client.indices.create(index=TEST_INDEX)
    self.contract_methods = ContractMethods(TEST_INDEX, ethereum_api_host='https://mainnet.infura.io/SuP0gwmZ0hYfutY70s6V')

  def test_iterate_contracts(self):
    self.client.index(index=TEST_INDEX, doc_type='contract', body={'address': TEST_CONTRACT_ADDRESSES[0]}, id=1, refresh=True)
    self.client.index(index=TEST_INDEX, doc_type='contract', body={'address': TEST_CONTRACT_ADDRESSES[1]}, id=2, refresh=True)
    self.client.index(index=TEST_INDEX, doc_type='contract', body={'address': TEST_CONTRACT_ADDRESSES[2]}, id=3, refresh=True)
    iterator = self.contract_methods.iterate_contracts()
